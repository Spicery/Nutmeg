...
define global constant procedure ispushable(x); lvars x;
    x.isclosure and pdpart(x) == canpush
enddefine;

define global constant procedure isemptypushable( G ); lvars G;
    if G.ispushable do
        not( fast_cont( frozval( 2, G ) ) )
    else
        mishap( 'Not a "newpushable" repeater', [^G] )
    endif
enddefine;
*/
